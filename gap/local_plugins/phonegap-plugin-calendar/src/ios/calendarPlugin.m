...
- (void)createEventInStore:(EKEventStore *)store command:(CDVInvokedUrlCommand*)command;
{
    ...
    [dateFormatterWithTimeZone setDateFormat:@"yyyy-MM-dd'T'HH:mm:ssZZZZZ"];
		...
		NSTimeInterval unixTimeStamp = [startDate doubleValue] / 1000;
    NSDate *tempDate = [NSDate dateWithTimeIntervalSince1970:unixTimeStamp];
    startDate = [dateFormatterWithTimeZone stringFromDate:tempDate];
    ...
    if (!myStartDate) {
		...
		unixTimeStamp = [endDate doubleValue] / 1000;
    tempDate = [NSDate dateWithTimeIntervalSince1970:unixTimeStamp];
    endDate = [dateFormatterWithTimeZone stringFromDate:tempDate];
